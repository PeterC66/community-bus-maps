# Changelog archive — everything up to 2026-08-19

This is the tail of `CHANGELOG.md` as it stood on 2026-08-27, from the `Security / CI — the P0 block of the technical audit — 2026-08-19` entry back to the first `[0.0.1-P0]` release of 2026-07-23. It was split out because the working changelog had reached 2,407 lines and the last seven days are what anyone actually reads; keeping the rest in the same file made the file expensive to load and impossible for two sessions to append to at once.

**It was archived rather than deleted, and the reason is worth knowing before anyone tidies it away.** Three documents cite entries below *by name*, and deleting the text would have left all three still resolving and still lying: `CLAUDE.md` cites the "impeccable round-2 findings" entry for the `--accent-tint-ink` token; `docs/DEPLOY.md`'s deploy history cites "the live host rendered every sheet in monospace" for the `fontconfig` fix at `3254c02`; and `docs/ROADMAP.md` sends readers here for the **P0–P5** lessons learned, which are the `[0.1.0-P1]` through `[0.5.0-P5]` sections. All three now point at this file.

Nothing here is maintained. New entries go to `CHANGELOG.d/` as fragments — see [`../../CHANGELOG.d/README.md`](../../CHANGELOG.d/README.md).
