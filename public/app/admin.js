...
function rowCust(c) {
  ...
  return `<div class="gt-row" role="row" data-cust="${c.id}" data-edit-key="cust-${c.id}">
    ...
    <div class="gt-cell" role="cell"><input type="checkbox" data-q="hideOps" title="Lets this organisation hide an operator in its map editor. Hiding one hides that operator's buses from their published map - readers will not see them."${c.hideOperatorsEnabled ? ' checked' : ''}></div>
