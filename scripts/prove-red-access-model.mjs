...
const MUTATIONS = [
  ...
  },
  {
    ...
    // Re-anchored 2026-09-02 (OA-231): the admin console is src/routes/admin.js now.
  ...
  {
    what: "the admin plugin's ONE guard accepts any signed-in user, so the owner route does too",
    why: 'a route that re-homes an asset between tenants is not one an editor may travel. Since OA-231 the owner route has no guard of its own: the plugin-level preHandler in src/routes/admin.js is the only door to all 22 admin routes, so this is the mutation that opens it',
    file: 'src/routes/admin.js',
    find: '    if (!requireAdmin(req, reply)) return reply;',
    to: "    if (!req.user) { reply.code(401).send({ ok: false, error: 'Please sign in.' }); return reply; }",
