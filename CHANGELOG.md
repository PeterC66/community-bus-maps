...
- **A new advisory the audit did not have.** `@fastify/static` picked up four highs (path traversal, route-guard bypass, authorisation bypass via non-canonical paths) after that document was written. The fix is `>=10.1.2` against the `8.3.0` in the lockfile — two majors, on the component serving every file under `public/`. Deferred with a short window, not waived; see `scripts/audit-allowlist.json` for the exposure assessment.
